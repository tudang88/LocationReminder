...
class RemindersActivityTest :
    ...
    fun addNewReminder() = runBlocking {
        // 1. start activity with empty list

        val activityScenario = ActivityScenario.launch(RemindersActivity::class.java)
        dataBindingIdlingResource.monitorActivity(activityScenario)
        // 2. click add button
        onView(withId(R.id.addReminderFAB)).perform(click())
        // 3. open SaveReminderFragment and input test data
        onView(withId(R.id.reminderTitle)).perform(replaceText("TITLE_1"))
        onView(withId(R.id.reminderDescription)).perform(replaceText("DESCRIPTION_1"))
        // set test POI
        saveReminderViewModel.updateSelectedPoi(
            PointOfInterest(
                LatLng(90.0, 90.0),
                "",
                "LOCATION_1"
    ...
    @Test
    fun selectReminderOnList_EditReminder_SaveReminder() = runBlocking {
        ...
        // 5. open SaveReminderFragment then change title
        onView(withId(R.id.reminderTitle)).perform(replaceText("TITLE_1_UPDATE"))
        onView(withId(R.id.saveReminder)).perform(click())
        // return to list
        onView(withText("TITLE_1_UPDATE")).check(matches(isDisplayed()))
        activityScenario.close()
    }
}
